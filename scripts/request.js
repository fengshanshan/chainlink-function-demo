...
async function main() {
  ...
  // Consumer contract
  const consumerAddress = "0xa6d51377d1b6f3c8570D024f974e669C69ef0929"
  const consumerAbiPath = "./build/artifacts/contracts/FunctionsConsumer.sol/FunctionsConsumer.json"
  ...
  if (typeof secrets !== "undefined" && typeof secretsLocation !== "undefined") {
    encryptedSecrets = await getEncryptedSecrets(secrets, secretsLocation, oracle, signerPrivateKey)
  } else {
    encryptedSecrets = "0x"
    secretsLocation = 0
  }
...
async function getEncryptedSecrets(secrets, secretsLocation, oracle, signerPrivateKey = null) {
  ...
  if (secretsLocation == 0 && Object.keys(secrets).length > 0) {
    ...
    return "0x" + (await (0, encryptWithSignature)(signerPrivateKey, DONPublicKey, JSON.stringify(secrets)))
  }
